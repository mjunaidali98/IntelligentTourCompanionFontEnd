...
export default class Manualselecthotels extends Component {
    ...
    initMap = (Hotels) => {
        ...
        for (var i = 0; i < Hotels.length; i++) {
            ...
            var hotelDetails = '<ul>' + '<li>' + '<strong>' + Hotels[i].name + '</strong>' + '</li>' + '<li>' + '<strong>Address: </strong>' + Hotels[i].address + '</li>' + '<li>' + '<strong>Ratings: </strong>' + Hotels[i].rating + " /5" + '</li>' + '<li>' + '<strong>Total Ratings: </strong>' + Hotels[i].totalRating + " people" + '</li>' + '<li>' + '<strong>Price: </strong>' + Hotels[i].price + " people" + '</li>' + '</ul>'
    ...
    async componentDidMount() {
        ...
        axios
            .get(
                `https://tripadvisor1.p.rapidapi.com/hotels/list?offset=0&currency=PKR&limit=30&order=asc&lang=en_US&sort=recommended&location_id=${this.state.Lat}%252C${this.state.Lng}&adults=1&checkin=2020-12-13&rooms=1&nights=2&maxResults=24`,
                {
                    method: 'GET',
                    headers: {
                ...
                response.data.data.map((hotel, index) => ({
                    ...
                    console.log('Image is ' + response.data.data[0].photo.images.small.url),
        ...
        return (
            <div>
                <SelectedHotels.Provider value={'zeeshan'}>
                    {this.state.loading == true ? <div style={{ marginLeft: '35%', position: 'relative', height: '300px', width: '600px' }}> <Spin style={{ marginTop: '30%' }} /></div> : <div><div id="mapcanvas" style={{ marginLeft: '12%', position: 'relative', height: '300px', width: '600px' }}>
                        <ToastContainer
                    ...
                    <p style={{ color: 'white', fontFamily: 'sans-serif', float: 'left', marginLeft: '3%', marginTop: '3%', padding: '8px', backgroundColor: '#001529', height: '40%', width: '13.2%', opactity: '0.9', boxShadow: '10px, 10px, 5px,#ccc' }}>We found <span style={{ color: 'red', fontWeight: 'bold', fontSize: '20px' }}>{this.state.loading == true ? <Spin style={{ marginLeft: '2%' }} /> : this.state.Hotels.length}</span><br /> hotels in <span style={{ fontWeight: 'bold' }}>{this.state.destination}</span></p>
                    <div id="allHotels" style={{ marginTop: '3%', height: '200px', width: '820px', overflowY: 'auto', overflowX: 'hidden' }}>
                        {this.state.Hotels.map((hotel) => <div className="card mb-3" style={{ boxShadow: '10px, 10px, 5px,#ccc', height: '100%', width: '810px', display: 'inline-block' }}>
                            <div className="row no-gutters">
                                <div className="col-md-4">
                                    <img
                                        ...
                                        className="card-img" style={{ height: '198px' }} />
                                </div>
                                <div className="col-md-8">
                                    <div className="card-body">
                                        <h4 className="card-title" style={{ color: '#001529', wordBreak: 'break-all' }}>{hotel.name}</h4>
                                        {hotel.rating === undefined ? <p style={{ textAlign: 'right', marginTop: '-6%', color: '#001529' }}>Not available</p> : <p style={{ textAlign: 'right', marginTop: '-6%', color: '#001529' }}>{hotel.rating > 4.5 ? 'Excellent' : hotel.rating > 3.5 ? 'Good' : 'Fine'}<span style={{ backgroundColor: '#001529', borderRadius: '5px', color: 'white', padding: '5px' }}><strong >{hotel.rating}</strong></span></p>}
